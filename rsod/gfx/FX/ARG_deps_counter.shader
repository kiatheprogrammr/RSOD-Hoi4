...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float totalDeps = floor(value/10000.f);
			float runningTally = floor(value/10.f) - (totalDeps * 1000.f);
			float colour = floor(value) - (totalDeps * 10000.f) - (runningTally * 10.f);
			...
			float timePos = floor(194.f * sinTime);
			
			float startingAmount = (runningTally - totalDeps);
			if(startingAmount > timePos && vTime < 6.5){
				totalDeps = 0.f;
			}
			else if(startingAmount < timePos && runningTally > timePos && vTime < 6.5){
				totalDeps = timePos - startingAmount;
			}
			else{
				totalDeps = floor(value/10000.f);
			}
