...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float4 firstPartyColour = tex2D(TextureTwo, float2(((firstParty / 9.f) - 0.001f), 0.0f));
			float4 secondPartyColour = tex2D(TextureTwo, float2(((secondParty / 9.f) - 0.001f), 0.0f));
